...
title: OpenAI-Compatible API Server for Drewgent Agent
type: document
space: concept
tags: [concept]
created: 2026-05-20
...
# OpenAI-Compatible API Server for Drewgent Agent
...
SSE streaming. By exposing this endpoint, drewgent-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  drewgent-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.drewgent/.env`
2. Run `drewgent gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with drewgent-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns drewgent-agent as a model) |
...
Authorization: Bearer drewgent-api-key-here
...
But drewgent-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "drewgent-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured drewgent-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of drewgent-agent's core prompt
...
Once implemented, drewgent-agent works as a drop-in backend for:
