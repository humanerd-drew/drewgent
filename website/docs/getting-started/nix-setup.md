...
Drewgent Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`drewgent setup`, `drewgent gateway install`, config editing) works identically to the standard install.
...
After `nix profile install`, `drewgent``, `drewgent-agent`, and `drewgent-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `drewgent setup` walks you through provider selection, `drewgent gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.drewgent/`.
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, drewgent-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        drewgent-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
That's it. `nixos-rebuild switch` creates the `drewgent`` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```nix
services.drewgent-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `drewgent`` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `drewgent`` in your shell creates a separate `~/.drewgent/` directory.
:::
...
systemctl status drewgent-agent

# Watch logs (Ctrl+C to stop)
journalctl -u drewgent-agent -f
...
```nix
# base.nix
services.drewgent-agent.settings = {
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Drewgent defaults to OpenRouter.
:::
...
```nix
services.drewgent-agent.configFile = /etc/hermes/config.yaml;
```
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."drewgent-env".path ]` |
...
| Override the hermes package | `package` | `inputs.drewgent-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Drewgent reads this file on every startup, so changes take effect with a `systemctl restart drewgent-agent` — no container recreation needed.
...
```nix
{
  sops = {
    ...
    secrets."drewgent-env" = { format = "yaml"; };
  };

  services.drewgent-agent.environmentFiles = [
    config.sops.secrets."drewgent-env".path
  ];
}
```
...
```nix
{
  age.secrets.drewgent-env.file = ./secrets/drewgent-env.age;

  services.drewgent-agent.environmentFiles = [
    config.age.secrets.drewgent-env.path
  ];
}
```
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Drewgent looks for specific filenames by convention:

- **`SOUL.md`** — the agent's system prompt / personality. Drewgent reads this on startup and uses it as persistent instructions that shape its behavior across all conversations.
- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

```nix
{
  services.drewgent-agent.documents = {
    "SOUL.md" = ''
...
```nix
{
  services.drewgent-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.drewgent-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Drewgent implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.drewgent-agent.mcpServers.my-oauth-server = {
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Drewgent prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u hermes` (native):

```bash
# Container mode
docker exec -it drewgent-agent \
...
```bash
hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.drewgent/mcp-tokens/my-oauth-server{,.client}.json \
...
```nix
{
  services.drewgent-agent.mcpServers.analysis = {
    command = "npx";
...
| Blocked command | Why |
|---|---|
| `drewgent setup` | Config is declarative — edit `settings` in your Nix config |
| `drewgent` config edit` | Config is generated from `settings` |
| `drewgent` config set <key> <value>` | Config is generated from `settings` |
| `drewgent gateway install` | The systemd service is managed by NixOS |
| `drewgent gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it drewgent-agent hermes config set ...` is also blocked)
...
Host                                    Container
────                                    ─────────
/nix/store/...-drewgent-agent-0.1.0  ──►  /nix/store/... (ro)
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/drewgent-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `drewgent`` and `drewgent-agent` binaries exist and `drewgent` version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `drewgent --help` exposes `gateway` and `config` subcommands |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the drewgent-agent service |
| `package` | `package` | `drewgent-agent` | The drewgent-agent package to use |
...
| `addToSystemPackages` | `bool` | `false` | Add `drewgent`` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `drewgent gateway` |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Drewgent binary + all Nix deps |
...
nix flake update drewgent-agent --flake /etc/nixos
...
```bash
systemctl status drewgent-agent
docker ps -a --filter name=drewgent-agent
docker inspect drewgent-agent --format='{{.State.Status}}'
docker exec -it drewgent-agent bash
docker exec drewgent-agent readlink /data/current-package
docker exec drewgent-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop drewgent-agent
docker rm -f drewgent-agent
sudo rm /var/lib/hermes/.container-identity
sudo systemctl start drewgent-agent
```
...
# Container mode
docker exec drewgent-agent cat /data/.hermes/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec drewgent-agent readlink /data/current-package)
```
...
| `drewgent` version` shows old version | Container not restarted | `systemctl restart drewgent-agent` |
