...
Drewgent deliberately separates:
...
`SOUL.md` lives at `~/.drewgent/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_drewgent_md(cwd_path)       # 1. .drewgent.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.drewgent.md`, `HERMES.md` | CWD up to git root | Drewgent-native project config |
...
- **YAML frontmatter stripped** — `.drewgent.md` frontmatter is removed (reserved for future config overrides)
...
1. `.drewgent.md` / `HERMES.md` (walks to git root)
