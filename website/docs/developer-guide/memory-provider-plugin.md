...
Memory provider plugins give Drewgent Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          drewgent_home (str): Active HERMES_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `drewgent` memory setup` | **Yes** |
| `save_config(values, drewgent_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `drewgent` memory setup`:
...
```python
def save_config(self, values: dict, drewgent_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(drewgent_home) / "my-provider.json"
...
All storage paths **must** use the `drewgent_home` kwarg from `initialize()`, not hardcoded `~/.drewgent`:

```python
# CORRECT — profile-scoped
from drewgent_constants import get_drewgent_home
data_dir = get_drewgent_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.drewgent/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `drewgent` my-provider status`, `drewgent` my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `drewgent` <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `drewgent --help`.
