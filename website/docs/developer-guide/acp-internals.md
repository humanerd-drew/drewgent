...
The ACP adapter wraps Drewgent' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
drewgent acp / drewgent-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.drewgent/.env
  -> configure stderr logging
  -> construct DrewgentACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Drewgent `once`
- `allow_always` -> Drewgent `always`
- reject options -> Drewgent `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Drewgent tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["drewgent-acp"])
...
Instead it reuses Drewgent' runtime resolver:

- `acp_adapter/auth.py`
- `drewgent_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Drewgent provider/credentials.
...
- `toolsets.py` — `drewgent-acp` toolset definition
- `drewgent_cli/main.py` — `drewgent acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `drewgent-acp` script
