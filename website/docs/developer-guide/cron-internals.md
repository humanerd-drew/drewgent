...
| `drewgent_cli/cron.py` | CLI `drewgent` cron` subcommands |
...
Jobs are stored in `~/.drewgent/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `drewgent` cron` commands are run or during active CLI sessions.
...
```python
# ~/.drewgent/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.drewgent/cron/output/` |
...
The `drewgent` cron` CLI provides direct job management:
