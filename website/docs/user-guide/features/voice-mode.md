...
Drewgent Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Drewgent](/docs/guides/use-voice-mode-with-hermes).

## Prerequisites

Before using voice features, make sure you have:

1. **Drewgent Agent installed** — `pip install drewgent-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `drewgent` model` or set your preferred provider credentials in `~/.drewgent/.env`
3. **A working base setup** — run `drewgent`` to verify the agent responds to text before enabling voice

:::tip
The `~/.drewgent/` directory and default `config.yaml` are created automatically the first time you run `drewgent``. You only need to create `~/.drewgent/.env` manually for API keys.
:::
...
pip install "drewgent-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "drewgent-agent[all]"
```
...
1. Start the CLI with `drewgent`` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.drewgent/config.yaml` (default: `ctrl+b`).
:::
...
```bash
drewgent gateway        # Start the gateway (connects to configured platforms)
drewgent gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.drewgent/.env`:
```bash
...
```bash
drewgent gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.drewgent/.env`
...
- Review gateway logs: `tail -f ~/.drewgent/logs/gateway.log`
