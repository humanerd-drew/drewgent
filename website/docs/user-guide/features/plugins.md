...
Drewgent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a Drewgent Plugin](/docs/guides/build-a-drewgent-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.drewgent/plugins/` with a `plugin.yaml` and Python code:

```
~/.drewgent/plugins/my-plugin/
...
Start Drewgent — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.drewgent/plugins/hello-world/plugin.yaml`**
...
**`~/.drewgent/plugins/hello-world/__init__.py`**

```python
"""Minimal Drewgent plugin — registers a tool and a hook."""
...
Drop both files into `~/.drewgent/plugins/hello-world/`, restart Drewgent, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Drewgent.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `drewgent` <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | Copy `skill.md` to `~/.drewgent/skills/` at load time |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `drewgent` plugins install` |
| Distribute via pip | `[project.entry-points."drewgent_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| User | `~/.drewgent/plugins/` | Personal plugins |
| Project | `.hermes/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `drewgent_agent.plugins` entry_points | Distributed packages |
...
Running `drewgent` plugins` with no arguments launches an interactive curses checklist (same UI as `drewgent` tools`) where you can toggle plugins on/off with arrow keys and space.
...
See the **[full guide](/docs/guides/build-a-drewgent-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
