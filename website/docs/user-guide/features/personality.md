...
Drewgent Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Drewgent is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Drewgent now seeds a default `SOUL.md` automatically in:

```text
~/.drewgent/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Drewgent with a custom home directory, it will use:
...
- Drewgent creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Drewgent loads `SOUL.md` only from `HERMES_HOME`
- Drewgent does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Drewgent falls back to a built-in default identity
...
If Drewgent loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Drewgent instance itself.

That also makes it easier to teach users:
- "Edit `~/.drewgent/SOUL.md` to change Drewgent' default personality."
...
- how Drewgent should handle uncertainty, disagreement, or ambiguity
...
## What Drewgent injects into the prompt
...
Drewgent ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Drewgent, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Drewgent its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.drewgent/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.drewgent/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Drewgent speaks
- `display.skin` and `/skin` affect how Drewgent looks in the terminal
