...
The `execute_code` tool lets the agent write Python scripts that call Drewgent tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a sandboxed child process on the agent host, communicating via Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from drewgent_tools import ...`
2. Drewgent generates a `drewgent_tools.py` stub module with RPC functions
3. Drewgent opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Drewgent
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from drewgent_tools import web_search, web_extract
...
```python
from drewgent_tools import search_files, read_file
import json
...
```python
from drewgent_tools import search_files, read_file, patch
...
```python
from drewgent_tools import terminal, read_file
import json
...
**Rule of thumb:** Use `execute_code` when you need to call Drewgent tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
