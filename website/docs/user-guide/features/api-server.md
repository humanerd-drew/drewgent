...
The API server exposes drewgent-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to drewgent-agent and use it as a backend.
...
# Optional: only if a browser must call Drewgent directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "drewgent-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists `drewgent-agent` as an available model. Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), drewgent-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
