...
Drewgent Agent automatically snapshots your project before **destructive operations** and lets you restore it with a single command. Checkpoints are **enabled by default** — there's zero cost when no file-mutating tools fire.

This safety net is powered by an internal **Checkpoint Manager** that keeps a separate shadow git repository under `~/.drewgent/checkpoints/` — your real project `.git` is never touched.
...
- Drewgent detects when tools are about to **modify files** in your working tree.
...
Checkpoints are enabled by default. Configure in `~/.drewgent/config.yaml`:
...
Drewgent responds with a formatted list showing change statistics:
...
To keep checkpointing safe and fast, Drewgent applies several guardrails:

- **Git availability** — if `git` is not found on `PATH`, checkpoints are transparently disabled.
- **Directory scope** — Drewgent skips overly broad directories (root `/`, home `$HOME`).
...
- **Combine with Git worktrees** for maximum safety — keep each Drewgent session in its own worktree/branch, with checkpoints as an extra layer.
