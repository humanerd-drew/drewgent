...
Drewgent Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Drewgent checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.drewgent/config.yaml`:
...
1. **CLI flag**: Start a session with `drewgent` --yolo` or `drewgent` chat --yolo`
...
Configure the timeout in `~/.drewgent/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.drewgent/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.drewgent/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.drewgent/config.yaml`:
...
:::tip
Use `drewgent` config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Drewgent controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.drewgent/.env`:
...
For more flexible authorization, Drewgent includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `drewgent` pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.drewgent/config.yaml`:
...
**Storage:** Pairing data is stored in `~/.drewgent/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Drewgent applies strict security hardening to every container.
...
Container resources are configurable in `~/.drewgent/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.drewgent/sandboxes/docker/<task_id>/`
...
When loaded, Drewgent checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
Paths are relative to `~/.drewgent/`. Files are mounted to `/root/.hermes/` inside the container.
...
| **terminal** (local) | Blocks explicit Drewgent infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Drewgent infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Drewgent integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.drewgent/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.drewgent/logs/` for unauthorized access attempts
10. **Keep updated** — run `drewgent` update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.drewgent/.env
...
terminal:
  ...
  ssh_user: "hermes"
  ssh_key: "~/.ssh/drewgent_agent_key"
```
