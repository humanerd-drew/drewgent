...
Drewgent Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Drewgent so each session has a clean, isolated working directory.

## Why Use Worktrees with Drewgent?

Drewgent treats the **current working directory** as the project root:

- CLI: the directory where you run `drewgent`` or `drewgent` chat`
...
git worktree add ../repo-feature feature/drewgent-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/drewgent-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Drewgent there:

```bash
cd ../repo-feature

# Start Drewgent in the worktree
hermes
```
...
git worktree add ../repo-experiment-a feature/drewgent-a
git worktree add ../repo-experiment-b feature/drewgent-b
```
...
Each Drewgent process:

- Works on its own branch (`feature/drewgent-a` vs `feature/drewgent-b`).
...
- Drewgent checkpoint data under `~/.drewgent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Drewgent experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/drewgent-checkpoints-docs`, `feature/drewgent-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Drewgent from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `drewgent -w` (Automatic Worktree Mode)

Drewgent has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `drewgent`/drewgent-<hash>`).
...
```bash
drewgent -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `drewgent -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Drewgent session its own clean checkout.
