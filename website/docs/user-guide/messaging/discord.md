...
Drewgent Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Drewgent Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how Drewgent behaves once it's in your server.

## How Drewgent Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Drewgent responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, Drewgent only responds when you `@mention` it. If you post in a channel without mentioning it, Drewgent ignores the message. |
...
| **Threads** | Drewgent replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, Drewgent isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), Drewgent stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to Drewgent without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

Drewgent on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Drewgent agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and Drewgent session policy.
...
So if Alice and Bob both talk to Drewgent in `#research`, Drewgent treats those as separate conversations by default even though they are using the same visible Discord channel.
...
Drewgent tracks running agents by session key.
...
3. Enter a name for your application (e.g., "Drewgent Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Drewgent Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Drewgent gateway).

## Step 7: Find Your Discord User ID

Drewgent Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
## Step 8: Configure Drewgent Agent
...
Add the following to your `~/.drewgent/.env` file:
...
:::tip
You can run `drewgent gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## Configuration Reference

Discord behavior is controlled through two files: **`~/.drewgent/.env`** for credentials and env-level toggles, and **`~/.drewgent/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Drewgent. `"all"` — accept all bot messages. |
| `DISCORD_REACTIONS` | No | `true` | When `true`, the bot adds emoji reactions to messages during processing (👀 when starting, ✅ on success, ❌ on error). Set to `false` to disable reactions entirely. |

### Config File (`config.yaml`)

The `discord` section in `~/.drewgent/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Drewgent. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
Add these to your `~/.drewgent/.env`:
...
Drewgent Agent supports Discord voice messages:
...
- **Discord voice channels**: Drewgent can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/docs/user-guide/features/voice-mode)
- [Use Voice Mode with Drewgent](/docs/guides/use-voice-mode-with-hermes)
...
**Cause**: The Drewgent gateway isn't running, or the token is incorrect.

**Fix**: Check that `drewgent gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.drewgent/.env` and restart the gateway.
...
**Fix**: Set this in `~/.drewgent/config.yaml` and restart the gateway:
...
For more information on securing your Drewgent Agent deployment, see the [Security Guide](../security.md).
