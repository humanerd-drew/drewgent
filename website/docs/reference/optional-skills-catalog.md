...
Official optional skills ship with the drewgent-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
drewgent skills install official/blockchain/solana
drewgent skills install official/mlops/flash-attention
```
...
```bash
drewgent skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Drewgent — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Drewgent via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Drewgent Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **drewgent-atropos-environments** | Build, test, and debug Drewgent Agent RL environments for Atropos training. Covers the DrewgentAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Drewgent phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
