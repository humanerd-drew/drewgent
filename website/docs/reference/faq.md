...
### What LLM providers work with Drewgent?
...
Set your provider with `drewgent` model` or by editing `~/.drewgent/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Drewgent Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.drewgent/`.

### Can I use it offline / with local models?

Yes. Run `drewgent` model`, select **Custom endpoint**, and enter your server's URL:
...
Drewgent persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Drewgent — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

Drewgent Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Drewgent Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Drewgent programmatically:

```python
from run_agent import AIAgent

agent = AIAgent(model="openrouter/nous/drewgent-3-llama-3.1-70b")
...
#### `drewgent`: command not found` after installation
...
**Cause:** Drewgent requires Python 3.11 or newer.
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.drewgent/.env` for conflicting entries.
:::
...
# Set a valid model
hermes config set HERMES_MODEL openrouter/nous/drewgent-3-llama-3.1-70b
...
- Using `drewgent` chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Drewgent detected the wrong context length for your model.
...
If this happens on the first long conversation, Drewgent may have the wrong context length for your model. Check what it detected:
...
**Cause:** Drewgent detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Drewgent never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `drewgent` chat`
...
# Start the gateway
drewgent gateway start

# Check logs for errors
cat ~/.drewgent/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `drewgent gateway setup`
- Check gateway logs: `cat ~/.drewgent/logs/gateway.log | tail -50`
...
Configure in `~/.drewgent/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "drewgent-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
**Solution:** The gateway captures your shell PATH when you run `drewgent gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
drewgent gateway install    # Re-snapshots your current PATH
drewgent gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `drewgent` chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `drewgent` chat -t "terminal"`
...
cd ~/.drewgent/drewgent-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.drewgent/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
# Restart Drewgent or reload MCP after config changes
hermes chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Drewgent](/docs/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Drewgent will report a timeout. Check the server's own logs (not just Drewgent logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `drewgent` profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `drewgent` update`?

`drewgent` update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `drewgent` update` once — it covers every profile on the machine.
...
There is no hard limit. Each profile is just a directory under `~/.drewgent/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Drewgent can route subagents to a different model automatically. Set this in `~/.drewgent/config.yaml`:
...
Now when you tell Drewgent "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Drewgent do this?

**Current limitation:** Drewgent profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Drewgent reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `drewgent skills config` settings don't seem to take effect.

**Solution:** Use `drewgent skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`drewgent gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Drewgent creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Drewgent to another machine
...
2. Copy your entire `~/.drewgent/` directory **except** the `drewgent-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='drewgent-agent' ~/.drewgent/ newmachine:~/.drewgent/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   hermes profile export default ./drewgent-backup.tar.gz

   # On target machine
   hermes profile import ./drewgent-backup.tar.gz default
   ```

3. On the new machine, run `drewgent setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.drewgent/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.drewgent/drewgent-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Drewgent installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Drewgent-specific issue — it's a shell config permissions problem.
