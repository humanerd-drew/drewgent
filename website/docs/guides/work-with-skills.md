...
Skills are on-demand knowledge documents that teach Drewgent how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Drewgent installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Drewgent to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Drewgent but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
drewgent skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.drewgent/skills/`
...
```bash
# Check it's there
drewgent skills list | grep arxiv
...
When a skill with config is first loaded, Drewgent prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
drewgent skills config gif-search
...
```bash
mkdir -p ~/.drewgent/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.drewgent/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.drewgent/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Drewgent may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Drewgent will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.drewgent/skills/devops/`, `~/.drewgent/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Drewgent to update the skill with what you learned. Skills that aren't maintained become liabilities.
