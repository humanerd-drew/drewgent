...
2. **Drewgent spins up** a fresh agent session with your prompt
...
- **Drewgent Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** — the gateway daemon handles cron execution:
  ```bash
  drewgent gateway install   # Install as a user service
  sudo drewgent gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.drewgent/cron/output/` and you can read them anytime.
:::
...
Drewgent will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Drewgent Agent
```
...
Just tell Drewgent what you want:
...
Drewgent will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Drewgent to delegate each topic to a sub-agent:
...
Drewgent will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
