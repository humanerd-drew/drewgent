...
This guide shows how to actually use MCP with Drewgent Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Drewgent tool
- you want Drewgent to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Drewgent to internal APIs, databases, or company systems without modifying Drewgent core

Do not use MCP when:
- a built-in Drewgent tool already solves the job well
...
- Drewgent remains the agent
- MCP servers contribute tools
- Drewgent discovers those tools at startup or reload time
...
If you installed Drewgent with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Drewgent banner/status should show MCP integration when configured
- ask Drewgent what tools it has available
...
There are two categories of MCP-exposed functionality in Drewgent:
...
2. Drewgent-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Drewgent will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Drewgent to reason over a bounded workspace.
...
Now Drewgent can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Drewgent core.
...
Because Drewgent now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
