...
`SOUL.md` is the **primary identity** for your Drewgent instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Drewgent to feel like the same assistant every time you talk to it — or if you want to replace the Drewgent persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Drewgent should be
- what Drewgent should avoid stylistically
- how Drewgent should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Drewgent is and how Drewgent speaks
...
Drewgent now uses only the global SOUL file for the current instance:

```text
~/.drewgent/SOUL.md
```

If you run Drewgent with a custom home directory, it becomes:
...
Drewgent automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Drewgent does not overwrite it
- if the file exists but is empty, Drewgent adds nothing from it to the prompt

## How Drewgent uses it

When Drewgent starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Drewgent feels.
...
Drewgent already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Drewgent should sound.

# Avoid
What Drewgent should not do.

# Defaults
How Drewgent should behave when ambiguity appears.
```
...
```bash
nano ~/.drewgent/SOUL.md
```

or

```bash
vim ~/.drewgent/SOUL.md
```

Then restart Drewgent or start a new session.
...
4. Talk to Drewgent for a while
...
### I edited SOUL.md but Drewgent still sounds the same

Check:
- you edited `~/.drewgent/SOUL.md` or `$HERMES_HOME/SOUL.md`
...
### Drewgent is ignoring parts of my SOUL.md
