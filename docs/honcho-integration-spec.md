...
Two independent Honcho integrations have been built for two different agent runtimes: **Drewgent Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Drewgent-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Drewgent that are worth propagating and patterns in openclaw-honcho that Drewgent should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Drewgent: baked-in runner
...
| Dimension | Drewgent Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Drewgent Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Drewgent are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.

**Drewgent contributes:**
...
**openclaw-honcho contributes back (Drewgent should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Drewgent
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Drewgent patterns from day one.
