...
_loaded_env_paths = load_drewgent_dotenv(drewgent_home=_drewgent_home, project_env=_project_env)
...
if tinker_atropos_dir.exists():
    ...
    # Fall back to drewgent-agent directory if submodule not found
...
from drewgent_constants import get_drewgent_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_drewgent_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.drewgent/config.yaml if not provided)
    ...
    """
    # Load config from ~/.drewgent/config.yaml
    config = load_drewgent_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.drewgent/.env")
            else:
