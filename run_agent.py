...
# ─── Extracted to core/runtime/ ──────────────────────────────────────────────
# These imports provide backwards-compatible stubs for extracted code.
from core.runtime.core import AIAgentInternals
from core.runtime.prompt import _build_system_prompt, _invalidate_system_prompt
from core.runtime.streaming import _interruptible_streaming_api_call
...
class AIAgent(AIAgentInternals):
    ...
    def __init__(
        ...
        # Drewgent is a tool-using client by default.
        ...
        # both live under ~/.drewgent/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from drewgent_logging import setup_logging, setup_verbose_logging
        from drewgent_constants import get_drewgent_home

        setup_logging(drewgent_home=get_drewgent_home())
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://drewgent-agent.nousresearch.com",
                        "X-OpenRouter-Title": "Drewgent Agent",
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `drewgent model`."
                        )
        ...
        # Session logs go into ~/.drewgent/sessions/ alongside gateway sessions
        drewgent_home = get_drewgent_home()
        self.logs_dir = drewgent_home / "sessions"
        ...
        try:
            from drewgent_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    ...
                    if _auto_learn_enabled:
                        _auto_learn_max = int(
                            mem_config.get("auto_learn_max_per_turn", 2)
                        )
                        from drewgent_constants import get_drewgent_home
                        ...
                        if not self.quiet_mode:
                            print(
                                f"🧠 Auto-learning: ENABLED (Obsidian wiki: {_wiki_path})"
                            )
            except Exception:
...
# Brain signal system: initialize signal processor + awareness reporter
            ...
            try:
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from drewgent_constants import get_drewgent_home as _ghh

                        _init_kwargs = {
                            "session_id": self.session_id,
                            "platform": platform or "cli",
                            "drewgent_home": str(_ghh()),
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "drewgent"
                        except Exception:
                            pass
    ...
    def run_conversation(
        ...
        # are reused.  The system prompt is Drewgent's territory; plugins
        ...
        while (
            ...
            # cache prefix.  The system prompt is reserved for Drewgent internals.
            if effective_system:
                api_messages = [
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(
                            f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Drewgent-managed OAuth/setup tokens"
                        )
                        print(
                        ...
                        print(
                            f'{self.log_prefix}     • Clear stale keys: drewgent config set ANTHROPIC_TOKEN ""'
                        )
                        print(
                            f'{self.log_prefix}     • Legacy cleanup: drewgent config set ANTHROPIC_API_KEY ""'
                        )
                    ...
                    if is_client_error:
                        ...
                        if (
                            ...
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(
                                    f"{self.log_prefix}      2. Then run `drewgent auth` to re-authenticate.",
                                    force=True,
                                )
                            else:
                                self._vprint(
                                ...
                                self._vprint(
                                    f"{self.log_prefix}      • Is the key valid? Run: drewgent setup",
                                    force=True,
                                )
                                self._vprint(
        ...
        if self._auto_learner and final_response and original_user_message:
            try:
                self._auto_learner.learn_from_turn(
