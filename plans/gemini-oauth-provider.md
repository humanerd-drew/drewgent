...
title: Gemini OAuth Provider — Implementation Plan
type: document
space: concept
tags: [concept]
created: 2026-05-20
...
## Client ID
- Need to register a "Desktop app" OAuth client on a HUMANERD GCP project
...
## Token Lifecycle
- Store at `~/.drewgent/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `drewgent_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `drewgent_cli/models.py` — Add Gemini model catalog
4. `drewgent_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `drewgent_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `drewgent_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- HUMANERD GCP project with Desktop OAuth client registered
...
- drewgent-agent Copilot OAuth: `drewgent_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
