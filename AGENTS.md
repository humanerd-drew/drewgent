...
~/.drewgent/                     # Drewgent 작업 디렉토리
  ...
  @memory/growth/trend-harvester/  # Trend harvester output (collected/applied/evaluated)
...
~/Library/LaunchAgents/          # launchd 서비스 (재부팅/크래시 자동 복구)
  ...
  ai.drewgent.cloudflared-wp.plist # Cloudflare Tunnel → humanerd.kr (WordPres)
```
...
| 매일 04:00 | log rotation, wiki-lint | shell / opencode run |
...
| 월 03:00 | wiki-compile | opencode run |
...
→ WordPress MU plugin으로 serve → https://humanerd.kr/.well-known/ai-catalog.json
```

- 각 MCP 툴: `capabilities`, `representativeQueries`, `metadata` 포함
- 호스팅: WordPress MU plugin (`humanerd-ard.php`)이 `/.well-known/ai-catalog.json` 인터셉트
