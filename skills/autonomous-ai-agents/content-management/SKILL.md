...
- **Use**: Surface notable Google updates, agentic web trends, or shifts in AI/discovery landscape → write analysis/opinion posts linking to humanerd.kr's own ARD integration
...
Before drafting, study `https://humanerd.kr/` `s published posts (ID 12, 13) to absorb Drew's actual voice.
...
Content-manager publishes to humanerd.kr (WordPress + GeneratePress) via the WordPress MCP server.
...
content-manager agent (every 3h)
  ...
  → editor agent runs QA (within 1h)
...
| Property | Value |
|----------|-------|
| URL | `https://humanerd.kr` |
| Admin | `https://humanerd.kr/wp-admin` |
