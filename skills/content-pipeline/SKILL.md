...
- "이 표현을 지워야 하나?" → "이 표현이 여기서 무슨 일을 하나?"로 바꿔서 판단
- `-적`, `의`, `들`, `것`, `있는/있다는`, `있었다` 등은 패턴 매칭으로 무조건 삭제하지 않고, 의미·리듬·강조·장르에 필요한 역할을 하는지 확인
- 개념어·전문용어·대조·소유·출처·실제 복수·고정 표현이면 유지

**변경률 가드:**
- 목표: 5~30%. 30% 초과 시 과윤문 가능성 재검토
- 50% 초과 시 강제 중단, 원문 유지, summary에 "변경률 50%+ — 사람 검토 권고" 표기
...
Draft 작성 완료 시 확인. 모든 패턴 매칭은 "삭제 규칙이 아니라 역할 확인" — 표현이 여기서 무슨 일을 하는지 먼저 판단하고 수정할 것.
