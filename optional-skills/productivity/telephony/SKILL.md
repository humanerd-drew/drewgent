...
This optional skill gives Drewgent practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.drewgent/.env`
...
It does **not** turn Drewgent into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Drewgent memory
...
### 1) "I want Drewgent to own a real phone number"
Use **Twilio**.
...
- pairs well with Drewgent `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.drewgent/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
drewgent skills search telephony
drewgent skills install official/productivity/telephony
```
...
Then save credentials into Drewgent:
...
3. Buy it and save it into `~/.drewgent/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Drewgent calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Drewgent's existing `text_to_speech` support.

Use this when:
- you want the call to use Drewgent's configured TTS voice rather than Twilio `<Say>`
...
Recommended Drewgent TTS -> Twilio Play workflow:

1. Generate the audio with Drewgent `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Drewgent to create the message audio with `text_to_speech`
...
Important note:
- Drewgent TTS is great for prerecorded outbound messages
...
- Drewgent STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Drewgent memory.
...
- Do not store arbitrary third-party phone numbers in Drewgent memory.
...
3. persist that number to `~/.drewgent/.env`
