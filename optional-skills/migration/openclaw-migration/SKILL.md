...
Use this skill when a user wants to move their OpenClaw setup into Drewgent Agent with minimal manual cleanup.
...
```bash
drewgent claw migrate              # Full interactive migration
...
drewgent claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `drewgent setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_drewgent.py` to:

- import `SOUL.md` into the Drewgent home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Drewgent memory entries
- merge OpenClaw command approval patterns into Drewgent `command_allowlist`
- migrate Drewgent-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.drewgent/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Drewgent workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.drewgent/tts/`
- archive non-secret docs that do not have a direct Drewgent destination
...
- `scripts/openclaw_to_drewgent.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py`

Do not guess a shorter path like `~/.drewgent/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.drewgent/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Drewgent-compatible secrets, currently:
...
Drewgent CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Drewgent. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Drewgent workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Drewgent skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.drewgent/skills/migration/openclaw-migration/scripts/openclaw_to_drewgent.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Drewgent unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Drewgent targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Drewgent destination. Unsupported auth blobs must still be reported as skipped.
...
- Drewgent persona state imported
- Drewgent memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.drewgent/skills/openclaw-imports/`
