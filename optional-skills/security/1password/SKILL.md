...
- `tmux` available for stable authenticated sessions during Drewgent terminal calls (desktop app flow only)
...
### Service Account (recommended for Drewgent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.drewgent/.env` (the skill will prompt for this on first load).
...
## Drewgent Execution Pattern (desktop app flow)

Drewgent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/drewgent-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/drewgent-op.sock"
