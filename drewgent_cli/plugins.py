"""
Drewgent Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.drewgent/plugins/<name>/``
2. **Project plugins** – ``./.hermes/plugins/<name>/`` (opt-in via
   ``DREW_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``drewgent_agent.plugins``
   entry-point group.
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``drewgent honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        if _env_enabled("DREW_ENABLE_PROJECT_PLUGINS"):
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``drewgent_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``drewgent tools`` TUI so plugin-provided toolsets appear
