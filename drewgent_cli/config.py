"""
Configuration management for Drewgent Agent.

Config files are stored in ~/.drewgent/ for easy access:
- ~/.drewgent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.drewgent/.env         - API keys and secrets

This module provides:
- drewgent config          - Show current configuration
- drewgent config edit     - Open config in editor
- drewgent config set      - Set a specific value
- drewgent config wizard   - Re-run setup wizard
"""
...
from drewgent_cli.colors import Colors, color
from drewgent_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_drewgent_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Drewgent is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in DREW_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade drewgent-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "drewgent update"


def format_managed_message(action: str = "modify this Drewgent installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Drewgent installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.drewgent-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Drewgent installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade drewgent-agent"
        )

    return (
        f"Cannot {action}: this Drewgent installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Drewgent."
    )
...
# Re-export from drewgent_constants — canonical definition lives there.
from drewgent_constants import get_drewgent_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_drewgent_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_drewgent_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into DREW_HOME if the user doesn't have one yet."""
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Drewgent
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Drewgent sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.drewgent/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for drewgent-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Drewgent Agent*" header.
    ...
    # Logging — controls file logging to ~/.drewgent/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Drewgent auth store)",
    ...
    # DREW_TOOL_PROGRESS and DREW_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.drewgent.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'drewgent setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Drewgent won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'drewgent doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DREW_TOOL_PROGRESS")
            old_mode = get_env_value("DREW_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DREW_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DREW_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DREW_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DREW_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: drewgent config set <key> <value>")
    ...
    # metadata.drewgent.config in their SKILL.md frontmatter.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.drewgent/config.yaml."""
    import copy
...
#   openai-codex (OAuth — drewgent auth) — OpenAI Codex
#   nous         (OAuth — drewgent auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.drewgent/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.drewgent/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Drewgent env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.drewgent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.drewgent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.drewgent/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.drewgent/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Drewgent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  drewgent config edit     # Edit config file", Colors.DIM))
    print(color("  drewgent config set <key> <value>", Colors.DIM))
    print(color("  drewgent setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: drewgent config set <key> <value>")
            print()
            print("Examples:")
            print("  drewgent config set model anthropic/claude-sonnet-4")
            print("  drewgent config set terminal.backend docker")
            print("  drewgent config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'drewgent config migrate' to add them")
        ...
        print("  drewgent config           Show current configuration")
        print("  drewgent config edit      Open config in editor")
        print("  drewgent config set <key> <value>   Set a config value")
        print("  drewgent config check     Check for missing/outdated config")
        print("  drewgent config migrate   Update config with new options")
        print("  drewgent config path      Show config file path")
        print("  drewgent config env-path  Show .env file path")
        sys.exit(1)
