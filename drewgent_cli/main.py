#!/usr/bin/env python3
"""
Drewgent CLI - Main entry point.

Usage:
    drewgent                     # Interactive chat (default)
    drewgent chat                # Interactive chat
    drewgent gateway             # Run gateway in foreground
    drewgent gateway start       # Start gateway as service
    drewgent gateway stop        # Stop gateway service
    drewgent gateway status      # Show gateway status
    drewgent gateway install     # Install gateway service
    drewgent gateway uninstall   # Uninstall gateway service
    drewgent setup               # Interactive setup wizard
    drewgent logout              # Clear stored authentication
    drewgent status              # Show status of all components
    drewgent cron                # Manage cron jobs
    drewgent cron list           # List cron jobs
    drewgent cron status         # Check if cron scheduler is running
    drewgent doctor              # Check configuration and dependencies
    drewgent honcho setup                    # Configure Honcho AI memory integration
    drewgent honcho status                   # Show Honcho config and connection status
    drewgent honcho sessions                 # List directory → session name mappings
    drewgent honcho map <name>               # Map current directory to a session name
    drewgent honcho peer                     # Show peer names and dialectic settings
    drewgent honcho peer --user NAME         # Set user peer name
    drewgent honcho peer --ai NAME           # Set AI peer name
    drewgent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    drewgent honcho mode                     # Show current memory mode
    drewgent honcho mode [hybrid|honcho|local]  # Set memory mode
    drewgent honcho tokens                   # Show token budget settings
    drewgent honcho tokens --context N       # Set session.context() token cap
    drewgent honcho tokens --dialectic N     # Set dialectic result char cap
    drewgent honcho identity                 # Show AI peer identity representation
    drewgent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    drewgent honcho migrate                  # Step-by-step migration guide: OpenClaw native → Drewgent + Honcho
    drewgent version             Show version
    drewgent update              Update to latest version
    drewgent uninstall           Uninstall Drewgent Agent
    drewgent acp                 Run as an ACP server for editor integration
    drewgent sessions browse     Interactive session picker with search

    drewgent claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (drewgent tools, drewgent setup, drewgent model) use
...
#
# Many modules cache DREW_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("DREW_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.drewgent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set DREW_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.drewgent/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".drewgent" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set DREW_HOME
    if profile_name is not None:
        try:
            from drewgent_cli.profiles import resolve_profile_env
            drewgent_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent drewgent from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["DREW_HOME"] = drewgent_home
...
load_drewgent_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `hermes` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from drewgent_logging import setup_logging as _setup_logging
...
from drewgent_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from drewgent_cli.config import get_env_path, get_drewgent_home, load_config
    from drewgent_cli.auth import get_auth_status

    # Determine whether Drewgent itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from drewgent_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_drewgent_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_drewgent_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Drewgent has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Drewgent to use their tokens.
    if _has_drewgent_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'drewgent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Drewgent isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  drewgent setup")
        print()

        from drewgent_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'drewgent setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from drewgent_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["DREW_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from drewgent_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from drewgent_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Drewgent?")
        print()
    ...
    session_dir = get_drewgent_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: drewgent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  drewgent gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Drewgent Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  drewgent gateway")
        ...
        print()
        print("  Or install as a service: drewgent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'drewgent whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from drewgent_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``drewgent model``) and the setup wizard
    ...
    from drewgent_cli.config import load_config, get_env_value
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from drewgent_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from drewgent_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from drewgent_cli.config import get_env_value, save_env_value
    ...
    from drewgent_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from drewgent_cli.config import get_env_value, save_config, save_env_value
    from drewgent_cli.nous_subscription import (
    ...
    from drewgent_cli.models import _PROVIDER_MODELS, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from drewgent_cli.codex_models import get_codex_model_ids
    from drewgent_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `drewgent auth` stores device_code tokens),
...
def _model_flow_custom(config):
    ...
    """
    from drewgent_cli.auth import _save_model_choice, deactivate_provider
    from drewgent_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from drewgent_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `drewgent model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from drewgent_cli.auth import _save_model_choice, deactivate_provider
    from drewgent_cli.config import save_env_value, load_config, save_config
    from drewgent_cli.models import fetch_api_models
...
from drewgent_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from drewgent_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from drewgent_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Drewgent turns to `copilot --acp`.")
    print("  Drewgent currently starts its own ACP subprocess for each request.")
    print("  Drewgent uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set DREW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from drewgent_cli.config import get_env_value, save_env_value, load_config, save_config
    from drewgent_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from drewgent_constants import display_drewgent_home as _dhh_fn
            print(f"    Drewgent will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'drewgent model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Drewgent CLI with a provider."""
    from drewgent_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from drewgent_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from drewgent_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from drewgent_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from drewgent_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from drewgent_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from drewgent_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from drewgent_cli.config import config_command
    config_command(args)
...
def cmd_version(args):
    ...
    try:
        ...
        from drewgent_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Drewgent Agent."""
    _require_tty("uninstall")
    from drewgent_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``drewgent update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Drewgent Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/drewgent-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="drewgent-update-")
        zip_path = os.path.join(tmp_dir, f"drewgent-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to drewgent-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"drewgent-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("drewgent-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Drewgent behaves unexpectedly.")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Drewgent couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Drewgent couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Drewgent behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `drewgent update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/drewgent-agent.git",
    "git@github.com:NousResearch/drewgent-agent.git",
    "https://github.com/NousResearch/drewgent-agent",
    "git@github.com:NousResearch/drewgent-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/drewgent-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from drewgent_constants import get_drewgent_home
    return (get_drewgent_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from drewgent_constants import get_drewgent_home
        (get_drewgent_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Drewgent repository.")
        print("  This means you may miss updates from NousResearch/drewgent-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/drewgent-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/drewgent-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``drewgent update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".drewgent"
    homes.append(default_home)
    # Named profiles under ~/.drewgent/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "drewgent-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Drewgent Agent to the latest version."""
    import shutil
    from drewgent_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Drewgent Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/drewgent-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_drewgent_home added to drewgent_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload drewgent_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_drewgent_home() added since the last release.
        try:
            import importlib
            import drewgent_constants as _hc
        ...
        try:
            from drewgent_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'drewgent config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: drewgent config migrate")
            else:
                print()
                print("Skipped. Run 'drewgent config migrate' later to configure.")
        else:
        ...
        try:
            from drewgent_cli.gateway import (
            ...
            # Discover all drewgent-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "drewgent-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. drewgent-gateway.service or drewgent-gateway-coder.service
            ...
            if is_macos():
                try:
                    from drewgent_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: drewgent gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: drewgent -p <profile> gateway run  for each profile)")
        ...
        print("  drewgent model              # Select provider and model")
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → drewgent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.drewgent)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  drewgent profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     drewgent -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from drewgent_constants import get_drewgent_home
                profile_dir_display = f"~/.drewgent/profiles/{name}"
        ...
        name = args.profile_name
        from drewgent_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from drewgent_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec drewgent -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from drewgent_cli.profiles import rename_profile
        try:
        ...
        from drewgent_cli.profiles import export_profile
        name = args.profile_name
        ...
        from drewgent_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from drewgent_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Drewgent log files."""
    from drewgent_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for drewgent CLI."""
    parser = argparse.ArgumentParser(
        prog="drewgent",
        description="Drewgent Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    drewgent                        Start interactive chat
    drewgent chat -q "Hello"        Single query mode
    drewgent -c                     Resume the most recent session
    drewgent -c "my project"        Resume a session by name (latest in lineage)
    drewgent --resume <session_id>  Resume a specific session by ID
    drewgent setup                  Run setup wizard
    drewgent logout                 Clear stored authentication
    drewgent auth add <provider>    Add a pooled credential
    drewgent auth list              List pooled credentials
    drewgent auth remove <p> <t>    Remove pooled credential by index, id, or label
    drewgent auth reset <provider>  Clear exhaustion status for a provider
    drewgent model                  Select default model
    drewgent config                 View configuration
    drewgent config edit            Edit config in $EDITOR
    drewgent config set model gpt-4 Set a config value
    drewgent gateway                Run messaging gateway
    drewgent -s drewgent-agent-dev,github-auth
    drewgent -w                     Start in isolated git worktree
    drewgent gateway install        Install gateway background service
    drewgent sessions list          List past sessions
    drewgent sessions browse        Interactive session picker
    drewgent sessions rename ID T   Rename/title a session
    drewgent logs                   View agent.log (last 50 lines)
    drewgent logs -f                Follow agent.log in real time
    drewgent logs errors            View errors.log
    drewgent logs --since 1h        Lines from the last hour
    drewgent update                 Update to latest version

For more help on a command:
    drewgent <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Drewgent Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: drewgent-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Drewgent Agent with an interactive wizard. "
                    "Run a specific section: drewgent setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Drewgent CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: drewgent-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Drewgent Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Drewgent Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Drewgent Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from drewgent_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from drewgent_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from drewgent_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/drewgent-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from drewgent_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from drewgent_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'drewgent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # drewgent tools list [--platform cli]
    ...
    # drewgent tools disable <name...> [--platform cli]
    ...
    # drewgent tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from drewgent_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from drewgent_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Drewgent as an MCP server",
        description=(
            "Manage MCP server connections and run Drewgent as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'drewgent mcp add' to connect to a new server, or\n"
            "'drewgent mcp serve' to expose Drewgent conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Drewgent as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from drewgent_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch drewgent --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            drewgent_bin = shutil.which("drewgent")
            if drewgent_bin:
                os.execvp(drewgent_bin, ["drewgent", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "drewgent_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Drewgent"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Drewgent",
    ...
    def cmd_claw(args):
        from drewgent_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Drewgent Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Drewgent Agent",
        description="Remove Drewgent Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Drewgent instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Drewgent log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    drewgent logs                    Show last 50 lines of agent.log
    drewgent logs -f                 Follow agent.log in real time
    drewgent logs errors             Show last 50 lines of errors.log
    drewgent logs gateway -n 100     Show last 100 lines of gateway.log
    drewgent logs --level WARNING    Only show WARNING and above
    drewgent logs --session abc123   Filter by session ID
    drewgent logs --since 1h         Lines from the last hour
    drewgent logs --since 30m -f     Follow, starting from 30 min ago
    drewgent logs list               List available log files with sizes
""",
    )
