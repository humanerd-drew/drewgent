...
Pure display functions with no DrewgentCLI state dependency.
"""
...
from drewgent_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.drewgent/.update_check``).  Returns the number of commits behind,
    or ``None`` if the check fails or isn't applicable.
    """
    drewgent_home = get_drewgent_home()
    repo_dir = drewgent_home / "drewgent-agent"
    cache_file = drewgent_home / ".update_check"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    agent_name = _skin_branding("agent_name", "Drewgent Agent")
