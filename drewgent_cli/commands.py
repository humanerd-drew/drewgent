"""Slash command definitions and autocomplete for the Drewgent CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update Drewgent Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``drewgent skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /drewgent handler.

    Maps both canonical names and aliases so /drewgent bg do stuff works
    the same as /drewgent background do stuff.
    """
    overrides = _resolve_config_gates()
