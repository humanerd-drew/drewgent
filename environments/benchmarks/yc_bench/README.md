...
title: YC-Bench: Long-Horizon Agent Benchmark
type: document
space: concept
tags: [concept]
created: 2026-05-20
...
The environment initialises the simulation via `yc-bench sim init` (NOT `yc-bench run`, which would start yc-bench's own built-in agent loop). Our `DrewgentAgentLoop` then drives all interaction through CLI commands.
