...
def heal_broken_links(entries: list, dry_run: bool = False) -> dict:
    ...
    for e in entries:
        ...
        for link in links:
            # Resolve: link could be "concept/foo" or just "foo"
            candidates = [
                MEMORIES / (link.replace("/", "-").replace(" ", "-") + ".md"),
                MEMORIES / (link.replace("/", "-").replace(" ", "-") + "/index.md"),
                MEMORIES / (link.split("/")[-1].replace(" ", "-") + ".md"),
            ]
            resolved = next((p for p in candidates if p.exists()), None)

            if not resolved:
                # Try fuzzy search in all known paths
                link_name = link.split("/")[-1].replace(" ", "-").lower()
                fuzzy = next((p for p in all_paths if link_name in p.name.lower()), None)
                if fuzzy:
