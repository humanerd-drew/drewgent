"""Anthropic Messages API adapter for Drewgent Agent.

Translates between Drewgent's internal OpenAI-style message format and
...
def build_anthropic_client(api_key: str, base_url: str = None):
    ...
    logging.warning(
        f"DEBUG: _anthropic_sdk={_anthropic_sdk}, file={getattr(_anthropic_sdk, '__file__', 'N/A')}, Anthropic={hasattr(_anthropic_sdk, 'Anthropic')}"
    )
...
def _prefer_refreshable_claude_code_token(
    ...
    Drewgent historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Drewgent)
    ...
    # 1. Drewgent-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Drewgent configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── Drewgent-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.drewgent/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_HERMES_OAUTH_FILE = get_drewgent_home() / ".anthropic_oauth.json"
...
def run_drewgent_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Drewgent-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Drewgent with your Claude Pro/Max subscription.")
    print()
...
def run_drewgent_oauth_login() -> Optional[str]:
    """Run Drewgent-native OAuth PKCE flow for Claude Pro/Max subscription.

    Opens a browser to claude.ai for authorization, prompts for the code,
    exchanges it for tokens, and stores them in ~/.drewgent/.anthropic_oauth.json.

    Returns the access token on success, None on failure.
    """
    result = run_drewgent_oauth_login_pure()
    if not result:
        return None
    ...
    _save_drewgent_oauth_credentials(access_token, refresh_token, expires_at_ms)
...
def _save_drewgent_oauth_credentials(
    access_token: str, refresh_token: str, expires_at_ms: int
) -> None:
    """Save OAuth credentials to ~/.drewgent/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        ...
        logger.debug("Failed to save Drewgent OAuth credentials: %s", e)


def read_drewgent_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Drewgent-managed OAuth credentials from ~/.drewgent/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Drewgent OAuth credentials: %s", e)
    return None


def refresh_drewgent_oauth_token() -> Optional[str]:
    """Refresh the Drewgent-managed OAuth token using the stored refresh token.

    Returns the new access token, or None if refresh fails.
    """
    creds = read_drewgent_oauth_credentials()
    ...
    try:
        ...
        _save_drewgent_oauth_credentials(
            refreshed["access_token"],
            refreshed["refresh_token"],
            refreshed["expires_at_ms"],
        )
        ...
        logger.debug("Successfully refreshed Drewgent OAuth token")
        return refreshed["access_token"]
    except Exception as e:
        logger.debug("Failed to refresh Drewgent OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Drewgent Agent", "Claude Code")
                text = text.replace("Drewgent agent", "Claude Code")
                text = text.replace("drewgent-agent", "claude-code")
