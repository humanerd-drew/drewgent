...
_DREW_MD_NAMES = (".drewgent.md", "DREW.md")


def _find_drewgent_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.drewgent.md`` or ``DREW.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Drewgent Agent, an intelligent AI assistant created by Nous Research. "
...
QA_GUIDANCE_TEMPLATE = (
    ...
    "CRITICAL: If all_criteria_met is FALSE, delivery will be BLOCKED and the user cannot see your output.\n"
    "Fill in REAL criteria — not placeholders. Real criteria make the QA gate meaningful.\n"
)
...
def _skills_prompt_snapshot_path() -> Path:
    return get_drewgent_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.drewgent/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("DREW_PLATFORM")
        or os.environ.get("DREW_SESSION_PLATFORM")
        or ""
    )
    cache_key = (
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    try:
        from drewgent_cli.nous_subscription import get_nous_subscription_features
    ...
    lines.extend(
        [
            ...
            "Useful commands: drewgent setup, drewgent setup tools, drewgent setup terminal, drewgent status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from DREW_HOME and return its content, or None.
    ...
    """
    try:
        from drewgent_cli.config import ensure_drewgent_home
        ensure_drewgent_home()
    except Exception as e:
        logger.debug("Could not ensure DREW_HOME before loading SOUL.md: %s", e)
...
def _load_drewgent_md(cwd_path: Path) -> str:
    ...
    if not drewgent_md_path:
        return ""
    try:
        content = drewgent_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = drewgent_md_path.name
        try:
            rel = str(drewgent_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".drewgent.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", drewgent_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .drewgent.md / DREW.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from DREW_HOME is independent and always included when present.
    ...
    # SOUL.md from DREW_HOME only — skip when already loaded as identity
    if not skip_soul:
