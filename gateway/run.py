...
_SESSION_CHECKPOINT_PATH = _drewgent_home / "session_checkpoint.json"
_SESSION_CHECKPOINT_TTL_DAYS = 7
...
def _load_gateway_config() -> dict:
    ...
    try:
        ...
        logger.debug(
            "Could not load gateway config from %s", _drewgent_home / "config.yaml"
        )
    return {}
...
class GatewayRunner:
    ...
    async def _pending_watcher_drain(self, interval: int = 30) -> None:
        """Background task that continuously drains pending_watchers.

        Picks up any new watchers registered by terminal(background=True) and
        starts _run_process_watcher tasks for them.  Runs every ``interval``
        seconds so background process notifications arrive even when no user
        messages are flowing in.
        """
        ...
        while self._running:
            try:
                ...
                logger.warning("Pending watcher drain error: %s", e)
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning(
                    "Slack: slack-bolt not installed. Run: pip install 'drewgent-agent[slack]'"
                )
                return None
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        # Write session checkpoint - mark as IN_PROGRESS (for crash recovery)
        self._write_session_in_progress(session_key, session_entry)
        ...
        try:
            ...
            # Mark session as COMPLETED in checkpoint (for crash recovery)
            try:
                self._write_session_complete(session_key)
            except Exception:
    ...
    async def _handle_brain_command(self, event: MessageEvent) -> str:
        """Handle /brain command — NeuronFS brain management.

        Supports:
          /brain init <name>      — Create a new brain scaffold
          /brain list             — List all brains
          /brain activate <name>  — Set active brain
          /brain emit            — Show current brain content
          /brain diag [name]     — Visualize brain structure
          /brain scan [name]     — Show brain statistics
          /brain fire <path>     — Strengthen a neuron
          /brain bomb <path>     — Disable a neuron path
          /brain unbomb <path>   — Re-enable a neuron path
        """
        ...
        try:
            ...
            if success:
                # Format for Discord/messaging: use code blocks for readability
                return f"```\n{output}\n```"
            else:
                # Show error with usage hint
                return f"❌ {output}\n\n**Usage:**\n`/brain init <name>` — Create brain\n`/brain activate <name>` — Set active\n`/brain emit` — Show content\n`/brain diag` — Visualize\n`/brain fire <path>` — Strengthen rule\n`/brain bomb <path>` — Disable rule"
        except Exception as e:
            logger.exception("Brain command failed")
            return f"⚠️ Brain command error: {e}"
    ...
    async def _watch_update_progress(
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        ...
                        await adapter.send(
                            chat_id,
                            "❌ Drewgent update failed (exit code {}).".format(
                                exit_code
                            ),
                        )
                    logger.info(
        ...
        if not exit_code_path.exists():
            ...
            try:
                await adapter.send(
                    chat_id, "❌ Drewgent update timed out after 30 minutes."
                )
            except Exception:
                pass
            for p in (
    ...
    def _clear_session_env(self) -> None:
        ...
        for var in [
            ...
            "DREW_SESSION_THREAD_ID",
        ]:
            if var in os.environ:
                del os.environ[var]

    # ------------------------------------------------------------------
    # Session Checkpoint (crash recovery for in-progress sessions)
    # ------------------------------------------------------------------

    def _write_session_in_progress(self, session_key: str, session_entry) -> None:
        """Mark a session as IN_PROGRESS in the checkpoint file."""
        try:
            checkpoint = self._read_session_checkpoint()
            checkpoint[session_key] = {
                "status": "IN_PROGRESS",
                "session_id": session_entry.session_id,
                "platform": session_entry.platform.value
                if session_entry.platform
                else None,
                "chat_id": session_entry.origin.chat_id
                if session_entry.origin
                else None,
                "thread_id": session_entry.origin.thread_id
                if session_entry.origin
                else None,
                "chat_type": session_entry.chat_type
                if hasattr(session_entry, "chat_type")
                else "dm",
                "started_at": time.time(),
                "updated_at": time.time(),
            }
            atomic_json_write(_SESSION_CHECKPOINT_PATH, checkpoint)
        except Exception as e:
            logger.warning("Failed to write session checkpoint (IN_PROGRESS): %s", e)

    def _write_session_complete(self, session_key: str) -> None:
        """Mark a session as COMPLETED in the checkpoint file."""
        try:
            checkpoint = self._read_session_checkpoint()
            if session_key in checkpoint:
                checkpoint[session_key]["status"] = "COMPLETED"
                checkpoint[session_key]["updated_at"] = time.time()
                atomic_json_write(_SESSION_CHECKPOINT_PATH, checkpoint)
            else:
                logger.warning(
                    "[Checkpoint] Session %s not found in checkpoint file — may have already been cleaned up or never marked IN_PROGRESS",
                    session_key[:30] if session_key else "unknown",
                )
        except Exception as e:
            logger.warning("Failed to write session checkpoint (COMPLETED): %s", e)

    def _read_session_checkpoint(self) -> Dict[str, Any]:
        """Read the session checkpoint file. Returns empty dict if none exists."""
        if not _SESSION_CHECKPOINT_PATH.exists():
            return {}
        try:
            return json.loads(_SESSION_CHECKPOINT_PATH.read_text(encoding="utf-8"))
        except Exception:
            return {}

    def _cleanup_session_checkpoint(self) -> int:
        """Remove expired COMPLETED entries older than TTL. Returns count of removed entries."""
        try:
            checkpoint = self._read_session_checkpoint()
            if not checkpoint:
                return 0

            cutoff = time.time() - (_SESSION_CHECKPOINT_TTL_DAYS * 86400)
            original_count = len(checkpoint)
            ...
            removed = original_count - len(checkpoint)
            if removed > 0:
                atomic_json_write(_SESSION_CHECKPOINT_PATH, checkpoint)
                logger.info(
                    "[Checkpoint] Cleaned up %d expired session entries", removed
                )

            return removed
        except Exception as e:
            logger.warning("Failed to cleanup session checkpoint: %s", e)
            return 0

    async def _auto_resume_interrupted_sessions(self) -> None:
        """On gateway startup, find IN_PROGRESS sessions and automatically resume them.

        This replaces the old _notify_interrupted_sessions which only notified users
        but required manual re-triggering. Now we automatically inject a synthetic
        resume message to continue the previous work.

        Supports all messaging platforms (Discord, Telegram, Slack, etc.) that
        have an active adapter registered.
        """
        await asyncio.sleep(3)  # Let adapters finish connecting first

        if not self.adapters:
            logger.warning(
                "[Recovery] No messaging adapters available — skipping session recovery"
            )
            return
        ...
        # Clean up expired COMPLETED entries before recovery
        ...
        logger.info(
            "[Recovery] Found %d interrupted session(s) to auto-resume", len(incomplete)
        )

        # Track which sessions have been successfully resumed (for atomic write)
        resumed_keys = []

        for session_key, data in incomplete:
            chat_id = data.get("chat_id")
            thread_id = data.get("thread_id")
            session_id = data.get("session_id")
            platform_str = data.get("platform")

            if not chat_id or not session_id or not platform_str:
                continue

            # Get the correct adapter for this platform
            try:
                platform_enum = Platform(platform_str)
            except ValueError:
                logger.warning(
                    "[Recovery] Unknown platform %r for session %s",
                    platform_str,
                    session_key[:30],
                )
                continue
            ...
            if not adapter:
                logger.debug(
                    "[Recovery] No adapter for platform %s — skipping session %s",
                    platform_str,
                    session_key[:30],
                )
                continue

            # Build thread-scoped metadata
            send_meta = {}
            if thread_id:
                send_meta = {"thread_id": thread_id}

            try:
                # Create a synthetic resume message to continue the session
                ...
                # Inject synthetic resume message via the correct platform adapter
                synth_event = MessageEvent(
                    text="[SYSTEM: Gateway가 재시동되었습니다. 이전 작업을 계속 이어서 진행해주세요. 세션 히스토리를 기반으로 동일한 맥락에서 응답해주세요.]",
                    message_type=MessageType.TEXT,
                    source=_source,
                )

                logger.info(
                    "[Recovery] Auto-resuming session: %s (%s) → %s",
                    session_key[:30],
                    platform_str,
                    chat_id,
                )
                ...
                # Only send notification AFTER successful recovery — prevents duplicate notifications on retry
                await adapter.send(
                    chat_id,
                    "🔄 **세션 복구 완료** — 이전 작업을 이어서 진행합니다.",
                    metadata=send_meta if send_meta else None,
                )

                # Mark as resumed — will be written atomically after loop
                resumed_keys.append(session_key)

            except Exception as e:
                logger.warning(
                    "[Recovery] Failed to auto-resume session %s: %s — will retry on next restart",
                    session_key[:30],
                    e,
                )

        # Atomic write: only mark as COMPLETED sessions that were successfully resumed
        if not resumed_keys:
            return
        ...
        try:
            ...
            for key in resumed_keys:
                if key in checkpoint:
                    checkpoint[key]["status"] = "COMPLETED"
                    checkpoint[key]["updated_at"] = time.time()
            atomic_json_write(_SESSION_CHECKPOINT_PATH, checkpoint)
            logger.info(
                "[Recovery] Checkpoint updated for %d resumed sessions",
                len(resumed_keys),
            )
        except Exception as e:
            # Rollback: restore IN_PROGRESS status for sessions that were marked complete in-memory
            # This keeps in-memory state and checkpoint file consistent
            try:
                checkpoint = self._read_session_checkpoint()
                for key in resumed_keys:
                    if (
                        key in checkpoint
                        and checkpoint[key].get("status") == "COMPLETED"
                    ):
                        checkpoint[key]["status"] = "IN_PROGRESS"
                        checkpoint[key]["updated_at"] = time.time()
                atomic_json_write(_SESSION_CHECKPOINT_PATH, checkpoint)
                logger.warning(
                    "[Recovery] Rollback complete — %d sessions restored to IN_PROGRESS",
                    len(resumed_keys),
                )
            except Exception:
                pass  # Rollback also failed — nothing more we can do
            logger.error(
                "[Recovery] CRITICAL — failed to update checkpoint after successful resume: %s. "
                "In-memory state has been rolled back to IN_PROGRESS.",
                e,
            )
    ...
    async def _run_agent(
        ...
        if _raw_tp is False:
            _raw_tp = "off"
        progress_mode = _raw_tp or os.getenv("DREW_TOOL_PROGRESS_MODE") or "all"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Drewgent Gateway - Multi-platform messaging"
    )
