...
And add it to the `drewgent-gateway` composite:

```python
"drewgent-gateway": {
    "includes": [..., "drewgent-your-platform"]
}
```
...
## 12. Status Display (`drewgent_cli/status.py`)
...
## 13. Gateway Setup Wizard (`drewgent_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ drewgent_cli/ toolsets.py \
  --include="*.py" -l | sort -u
